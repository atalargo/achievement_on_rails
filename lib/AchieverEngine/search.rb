    ...
    module Search
        def self.get_available_for_user(options)
            ...
            achievements = Achievement.by_project(options[:project_id]).
                active.
                typed(options[:type]) #.all_less(validated_user_achs.map(&:achievement_id) + in_progress_user_achs.map(&:achievement_id)) #.includes(:parents, :children)

            achievementRelations = AchievementRelation.by_project(options[:project_id])

            validated_user_achs = AchieverEngine::Validated.get_for_user_and_achievements(options, achievements)
...
#             p achievements
            ...
            obtainable_ids = []
            reject_ids = []
            obtained = Hash[validated_user_achs.map(&:achievement_id).collect{|v| [v, true]}.flatten]
...
p options
            validated_user_achs = if options[:user_obtaineds]
                                        options[:user_obtaineds]
                                else
                                        AchieverEngine::Validated.get_for_user_and_achievements({:project_id => options[:project_id], :user_id => options[:user_id]}, all_achievements).all
                                end

            obtained_ids = validated_user_achs.map(&:achievement_id)
        ...
        def self.achievements_for(options, mode = nil, mongo_data = false)
            ...
            user_achi = case mode
                when 'available'
                    ...
                    availables
                when 'validated'
                    user_achs = AchieverEngine::Validated.get_for_user_and_achievements(:project_id => options[:project].id, :user_id => options[:user].id)
                    ...
                    if mongo_data
                        {:achievements => achs, :validated => user_achs}
                    else
                        achs
                    end
                when 'in_progress'
