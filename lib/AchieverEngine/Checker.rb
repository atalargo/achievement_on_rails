...
module AchieverEngine
    class Checker
        ...
        def initialize(in_progress, data)
            ...
            @in_progress_achievements[:in_progress_data] = reorder_mg_achievements(@in_progress_achievements[:in_progress_data])
        ...
        def available_achievements_check
            ...
            user_achievements = UserAchievement.by_project(project_id).by_user(user_id)
                                                .find_or_create_by(:project_id => project_id, :user_id => user_id)

            available_achievements = AchieverEngine::Search.achievements_for(
                                        :project => project_id, :user => user_id, :mode => AchieverEngine::Search::ACHIEVEMENT_AVAILABLE_MODE
            )
            ...
            available_achievements.each do |achievement|
                ...
                @data.each do |input_data|
                    if input_data[:type] == achievement.on_type #&& achievement.behaviour_type == 'badge'
                        result_behaviour = AchieverEngine::Behaviour.get_behaviour(achievement.behaviour_type).check(achievement, input_data)

                        if !result_behaviour[:obtained].nil?
                            ...
                            user_achievements.in_progress << result_behaviour[:in_progress]
                        end
